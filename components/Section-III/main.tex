...
\begin{itemize}
    ...
    \item \textbf{Tích hợp bên thứ 3}: 
        ...
        \end{itemize}
    \item \textbf{Hạ tầng và Triển khai (DevOps)}: Quy trình triển khai được tự động hóa (CI/CD) thông qua GitHub.
        \begin{itemize}
            \item \textbf{Backend (Railway)}: Tự động build và deploy container mỗi khi có cập nhật mã nguồn, cung cấp môi trường serverless linh hoạt và HTTPS mặc định.
            \item \textbf{Frontend (Vercel)}: Tối ưu hóa việc phân phối nội dung tĩnh qua mạng lưới Edge Network toàn cầu, giúp giảm độ trễ truy cập cho người dùng.
        \end{itemize}
\end{itemize}
